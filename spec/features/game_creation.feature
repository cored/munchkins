Feature: Game creation
  ...
  Scenario: 6 Players
    Given a game with name "GameSunday" exists
    And a player with name "Rafael" exists
    And a player with name "Rafael" is assign to game "GameSunday"
    And a player with name "Roger" exists
    And a player with name "Roger" is assign to game "GameSunday"
    And a player with name "Karina" exists
    And a player with name "Karina" is assign to game "GameSunday"
    And a player with name "Ramon" exists
    And a player with name "Ramon" is assign to game "GameSunday"
    And a player with name "Julio" exists
    And a player with name "Julio" is assign to game "GameSunday"
    And a player with name "Marco" exists
    And a player with name "Marco" is assign to game "GameSunday"
    When game "GameNight" deals cards
